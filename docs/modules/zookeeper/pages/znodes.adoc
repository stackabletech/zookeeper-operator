...
Have a look at the usage guide for ZNodes: xref:usage_guide/isolating_clients_with_znodes.adoc[] or the CRD reference for the {crd-ref}/zookeeper.stackable.tech/v1alpha1/zookeeperznode{crd-ref-release}[ZookeeperZnode {external-link-icon}^] CustomResource.
