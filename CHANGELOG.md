...
- All product containers now run with `securityContext.runAsNonRoot` set to `true` to improve security ([#XXXX]).
...
- Fix a longstanding problem of including empty `categories`, `shortNames` and `additionalPrinterColumns` in the CRDs,
  ...
  See [our internal issue](https://github.com/stackabletech/hdfs-operator/issues/626) and [the fix](https://github.com/kube-rs/kube/pull/2042) for details ([#XXXX]).
...
[#XXXX]: https://github.com/stackabletech/zookeeper-operator/pull/XXXX
