...
VERSION := $(shell cargo metadata --format-version 1 | jq -r '.packages[] | select(.name=="stackable-zookeeper-operator") | .version')
...
docker-build:
	docker build --force-rm --build-arg VERSION=${VERSION} -t "docker.stackable.tech/stackable/zookeeper-operator:${VERSION}" -f docker/Dockerfile .

docker-build-latest: docker-build
	docker tag "docker.stackable.tech/stackable/zookeeper-operator:${VERSION}" \
...
chart-clean:
	...
	rm -rf deploy/helm/zookeeper-operator/crds

version:
	yq eval -i '.version = strenv(VERSION) | .appVersion = strenv(VERSION)' /dev/stdin < deploy/helm/zookeeper-operator/Chart.yaml
...
crds:
	...
	cargo run --bin stackable-zookeeper-operator -- crd | yq eval '.metadata.annotations["helm.sh/resource-policy"]="keep"' - > deploy/helm/zookeeper-operator/crds/crds.yaml
